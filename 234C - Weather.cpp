...
int main()
{
	...
	vector <int > minus(n+1),plus(n+1);
	
	for(int i=0; i< n ; i++){
		minus[i+1] = minus[i] + (num[i]<=0) ;
		
	}
	for(int i=0; i < n ; i++){
		plus[i+1] = plus[i] + (num[i] >= 0) ;
	}

	int answer = 1e9 + 5 ;
	for(int i=2; i <= n ; i++){
		
		// left minus and right plus ....
		int need1 = plus[i-1] +  minus[n] - minus[i-1];
		min_self(answer,need1);
