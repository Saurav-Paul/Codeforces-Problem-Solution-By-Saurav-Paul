...
////////////////////////
string a , b;
int rght=0,wrong = 0,destinatiion=0;
void find_ans(int curr,int n,int now){
	if(curr==n){
		if(now==destinatiion){
			rght++;
		}
...
int main()
{
	...
	int sz=(int) a.size();
	for(int i=0; i<sz ; i++)
		destinatiion += (a[i]=='+') - (a[i]=='-');
	find_ans(0,sz,0);
	
	double answer = (double)(rght*1.0)/(rght+wrong);
	printf("%.10lf\n",answer);
