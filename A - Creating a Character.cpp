...
void bruteforce(ll str,ll inti , ll exp){
	ll cnt = 0;
	for(ll i = 0 ; i <= exp ; i++){
		//~ debug() << dbg(i+str) dbg(inti+(exp-i));
		if(inti+exp-i < i+str)
			cnt ++ ;
	}
	debug() << dbg(cnt);
...
void solve(){
	ll str, inti , exp; cin>> str >> inti >> exp;

	ll mini = (str + inti + exp)/2 + 1;
	
	ll maxi = str + exp;
	
	cout << min(exp+1,max(0LL,maxi-mini+1)) << endl; 
