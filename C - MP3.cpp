...
inline long long int find_k(int i,int n){

	return pow(2,i/n);
}


void solve(vector< pair< int, int > > &v , int k, int total){
	

	int n = v.size();
	int sum = 0;
	for(int i = 0 , l = min(k,n) ; i < l ; i++)
		sum += v[i].second;
...
int main()
{
	...
	for(int i = 1; i < n ; i++){
		...
		if(v[i-1] != v[i]){
			...
			ara.emplace_back(v[i-1],cnt);
			cnt = 1;
		}
		
	}
	ara.emplace_back(v[n-1],cnt);
	debug() << dbg(ara) dbg(k);
